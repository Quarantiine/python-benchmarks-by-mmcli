...
| **LaTeX & Export Support**   | 🟢 Only build with LaTeX + Unicode export                                                                                                                                                                                      | 🟡 Clean ASCII / Unicode Tree rendering & step breakdown cards                                                                                                                                                        | 🔴 None                                                                                                                              | 🔴 None                                                                                                                                                                                                                                           | 🔴 None                                                                                                                                                                                      |
| **Simplification Depth**     | 🟡 Cleans trivial `*1`/`+0` terms — **but** ✅ on the live test, left the same uncancelled common `(x³+2x)` factor as Opus and Flash. No evidence yet that any of the three has a genuinely deeper simplifier than the others. | 🟢 Multi-pass rule-based reduction (up to 15 convergence iterations): constant folding, zero/identity laws, log/exp cancellation (`ln(e^x) -> x`), double negation, like-term combining (`2x+3x -> 5x`).           | 🔴 Untested (disqualified before reaching this stage)                                                                                | 🟡 Same uncancelled-factor result as mmcli and Flash ✅                                                                                                                                                                                           | 🟡 Same uncancelled-factor result as mmcli and Opus ✅                                                                                                                                       |
| **Calculus Subsystems**      | 🟢 ✅ Verified full CAS: indefinite/definite integration & limits via L'Hôpital's Rule (`lim(x->0) sin(x²cos(3x))/(x³+2x)² = 0.25`)                                                                                            | 🟢 Symbolic differentiation, numerical Simpson's rule definite integration (`int -l -u`), limit estimation (`lim -p`), algebraic simplification (`simplify`), AST tree visualizer (`tree`).                         | 🔴 Differentiation only, and that couldn't be verified either                                                                        | 🟡 Differentiation only (no integration/limits attempted)                                                                                                                                                                                         | 🟡 Differentiation only (no integration/limits attempted)                                                                                                                                    |
...
| :--- | :--- | :--- |
| **`mmcli-flash`** | 🟢 **PASS**: Exact derivative; simplified $+2\cos(x)\cdot(-\sin(x))$ to $-2\cos(x)\sin(x)$ | 🟢 **PASS**: Exact derivative across $\ln$, $\tan$, and $\sqrt$; simplified double negation to $+\frac{2x}{2\sqrt{4-x^2}}$ |
| **`antigravity-flash`** | 🟢 **PASS**: Exact derivative; simplified $+2\cos(x)\cdot(-\sin(x))$ to $-2\cos(x)\sin(x)$ | 🟢 **PASS**: Exact derivative across $\ln$, $\tan$, and $\sqrt$; simplified double negation to $+\frac{2x}{2\sqrt{4-x^2}}$ |
| **`mmcli-flash-lite`** | 🟢 **PASS**: Exact derivative and full AST parse; simplified constants & powers cleanly | 🟢 **PASS**: Exact derivative across $\log$, $\tan$, and $\sqrt$ |
